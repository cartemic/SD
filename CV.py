...
from CJ2 import CJspeed2
from PostShock import PostShock_fr
import datetime
from numpy import zeros
from cantera import Reactor, ReactorNet, Solution
...
def explosion(gas, fname, out):
    ...
    for n in range(b):
        ...
        # R = gas_constant
        R = 8314  # SAME ISSUE AS IN THERMO, FIX THIS
        wt = gas.mean_molecular_weight

        sumT = 0.0
        for z in range(j):
            w = mw[z]
            e = R * out.T[n] * (hs[z] / w - 1 / wt)
            wd = wdot[z]
            sumT += e * wd * w
    ...
    # HMPWt = zeros(2, float)
    if d == MAX:
        print('Initial Gradient is Maximum - ' +
              'post shock temperature may be too low')
        return gas
    while d < MAX:
        k += 1
        d = temp_grad[k]
    out.ind_time = out.time[k]
    # k1 = k
    k = 0
    MAX10 = 0.1 * MAX
    d = temp_grad[0]
    ...
    if fname == 0:
        ...
        while d < MAX:
            ...
            if out.time[k] == 0:
                ...
                mint = 0
                maxT = max(out.T)+0.1*min(out.T)
                minT = min(out.T)-0.1*min(out.T)
                maxP = max(out.P)+0.1*min(out.P)
                minP = min(out.P)-0.1*min(out.P)
                # maxpw = HMPWt[1] + 0.1*HMPWt[1]
                # minpw = HMPWt[0] - 0.1*HMPWt[0]
        ...
        # no wifi, not sure what to do about file. This probably needs 'with'
        # also might need some checks to make sure there are no issues with
        # existing files etc.
        # outfile = file(fn, 'w')
        with outfile as open(fn, 'w'):
            ...
            outfile.write('Variables = "Time", "Temperature", ' +
                          '"Pressure", "temp_grad"\n')
            for i in range(b):
